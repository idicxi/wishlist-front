...
function wishlistWsBase(apiBaseUrl: string = API_BASE_URL): string {
  const base = getWsBaseUrl(apiBaseUrl).replace(/\/$/, '');
  if (base.includes('/ws/wishlists')) return base;
  return `${base}/ws/wishlists`;
}

const WS_BASE_URL = wishlistWsBase(API_BASE_URL);
...
export function useWishlistSocket(wishlistId: number | null) {
  ...
  useEffect(() => {
    if (!wishlistId || wishlistId <= 0) return;
